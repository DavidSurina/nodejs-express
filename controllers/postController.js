...
module.exports = {
    getById: async(req, res) => {
        const {id} = req.params;
        try {
            const answerDB = await pool.query('SELECT * FROM posts WHERE post_id = $1', [id]);
            res.json({
    ...
    getRatings: async(_, res) => {
        try {
            // best practice table name in double quotes
            const answerDB = await pool.query('SELECT * FROM "posts" ORDER BY rating DESC');
            res.json({
    ...
    getPostByTitle: async(req, res) => {
        ...
        try {
            const data = await pool.query(
              "SELECT * FROM posts WHERE title LIKE $1", [title]
            );
            console.log(data);
            res.json({
              code: 200,
